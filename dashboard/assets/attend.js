        ...
        function getLocation() {
            ...
            LocationService.getCurrentPosition({ highAccuracy: true, timeout: 15000 })
                .then(function (pos) {
                    ...
                    gpsState = 'ok';
                    var acc = Math.round(pos.accuracy);
                    document.getElementById('attendLocation').textContent = pos.lat.toFixed(5) + ',' + pos.lng.toFixed(5) + ' ±' + acc + 'm';
                    var locMsg = '✓ 位置已获取';
                    document.getElementById('locationStatus').innerHTML = locMsg;
                    document.getElementById('locationStatus').setAttribute('data-base', locMsg);
                    document.getElementById('locationStatus').className = 'attend-location-status got';
                    checkPolylineGeofence(pos.lat, pos.lng).then(function () { updateSubmitState(); }).catch(function () { updateSubmitState(); });
                })
                .catch(function (err) {
                    attendLocation = null;
                    gpsState = 'fail';
                    var msg = err.message || '获取失败';
        ...
        function _showGpsHelp() {
            if (typeof LocationService !== 'undefined' && LocationService.showPermissionGuide) {
                LocationService.showPermissionGuide();
            } else {
        ...
        function updateSubmitState() {
            ...
            // 打卡不限制，按钮始终可用（仅显示提示信息）
            btn.disabled = false;
            var hint = [];
            if (!gpsOk) hint.push('GPS异常');
            if (!fenceOk) hint.push('围栏外');
            if (!faceOk && !fingerOk) hint.push('未验证');
            btn.textContent = '确认打卡' + (hint.length > 0 ? '（' + hint.join('，') + '）' : '');
        }
        ...
        async function submitAttendance() {
            ...
            try {
                ...
                if (isLeave) {
                    // Leave/travel submission — backend handles workflow_status & verify_status
