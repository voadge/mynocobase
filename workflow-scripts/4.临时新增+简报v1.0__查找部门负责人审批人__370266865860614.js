async function findUpward(deptId, db) {
  var du = await db.getRepository('departmentsUsers').findOne({ filter: { departmentId: deptId, isOwner: true }, appends: ['user', 'user.roles'] });
  if (du) {
    var roleTitle = null;
    if (du.user && du.user.roles && du.user.roles.length) {
      roleTitle = du.user.roles[0].title || du.user.roles[0].name;
    }
    return { userId: du.userId, role: roleTitle || '部门负责人', deptId: deptId };
  }
