...
docker exec -e PGPASSWORD=$(cat /opt/noco-base/secrets/db_password.txt) "$APP_CONTAINER" psql -h postgres -U nocobase -d nocobase -t -A <<'SQL_EOF'
SELECT name, packageName, enabled, builtIn FROM public."applicationPlugins" ORDER BY name;
SQL_EOF

echo "---"
echo "Deleting existing entry (if any)..."
docker exec -e PGPASSWORD=$(cat /opt/noco-base/secrets/db_password.txt) "$APP_CONTAINER" psql -h postgres -U nocobase -d nocobase <<'SQL_EOF'
