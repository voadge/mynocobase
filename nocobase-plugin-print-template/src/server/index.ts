...
import PrintTemplatesCollection from './collections/print-templates';
import { HtmlRenderer } from './services/html-renderer';
import { ExcelExporter } from './services/excel-exporter';
import fs from 'fs';
import path from 'path';

const STORAGE_DIR = path.join(process.cwd(), 'storage', 'print-template');
...
module.exports = class PrintTemplatePlugin extends Plugin {
  async load() {
    const app = this.app;
    const db = this.db;

    // Register collection
    db.collection(PrintTemplatesCollection);
    ...
    app.use(async (ctx: any, next: () => Promise<void>) => {
      const reqPath: string = ctx.state.reqPath || ctx.path.replace(/^\/api/, '');
      const method = ctx.method;

      // ---- Admin pages ----
      if (method === 'GET') {
        if (reqPath === '/__pt__/admin') {
          return serveHtml(ctx, path.join(STORAGE_DIR, 'list.html'));
        }
        if (reqPath.startsWith('/__pt__/admin/edit')) {
          return serveHtml(ctx, path.join(STORAGE_DIR, 'edit.html'));
        }
    ...
    app.use(async (ctx: any, next: () => Promise<void>) => {
      ...
      if (!tpl) { ctx.status = 404; ctx.body = 'Template not found or disabled'; return; }

      const schema = tpl.templateSchema;
      if (!schema?.blocks?.length && !schema?.elements?.length) {
        ctx.status = 404; ctx.body = 'Template has no content configured'; return;
      }
      ...
      if (!record) { ctx.status = 404; ctx.body = 'Record not found'; return; }

      const data: any = { [tpl.mainCollection]: record, _user: ctx.state.currentUser, _now: new Date() };
...
// ---- Action handler ----
async function handleResourceAction(ctx: any, app: any, db: any, actionName: string) {
  ...
  switch (actionName) {
    ...
    case 'getCollectionSchema': {
      ...
      if (!name) { ctx.status = 400; ctx.body = { errors: [{ message: 'Missing name parameter' }] }; return; }
      const collection = db.getCollection(name);
      if (!collection) { ctx.status = 404; ctx.body = { errors: [{ message: `Collection [${name}] not found` }] }; return; }
      ctx.body = {
        data: {
          name: collection.name, title: collection.options?.title || collection.name,
          fields: toArray(collection.fields)
            .filter((f: any) => !isSystemField(f))
            .map((f: any) => ({
    ...
    case 'getFieldTree': {
      ...
      if (!collectionName) { ctx.status = 400; ctx.body = { errors: [{ message: 'Missing collectionName parameter' }] }; return; }
      const collection = db.getCollection(collectionName);
      if (!collection) { ctx.status = 404; ctx.body = { errors: [{ message: `Collection [${collectionName}] not found` }] }; return; }
    ...
    case 'getRelationFields': {
      ...
      if (!relCollection || !relationPath) { ctx.status = 400; ctx.body = { errors: [{ message: 'Missing collection or relationPath parameter' }] }; return; }
      const coll = db.getCollection(relCollection);
      if (!coll) { ctx.status = 404; ctx.body = { errors: [{ message: `Collection [${relCollection}] not found` }] }; return; }
      ...
      if (!relField || !['belongsTo', 'hasOne', 'hasMany', 'belongsToMany'].includes(relField.type)) {
        ctx.status = 400; ctx.body = { errors: [{ message: `Field [${relationPath}] is not a relation` }] }; return;
      }
      const targetName = relField.options?.target;
      if (!targetName) { ctx.status = 400; ctx.body = { errors: [{ message: `Field [${relationPath}] has no target collection` }] }; return; }
      const targetColl = db.getCollection(targetName);
      if (!targetColl) { ctx.status = 404; ctx.body = { errors: [{ message: `Target collection [${targetName}] not found` }] }; return; }
      ctx.body = {
        data: {
    ...
    case 'printTemplateExportItem': {
      ...
      if (!resourceName) ctx.throw(400, 'Missing resourceName');
    ...
    case 'printTemplatePreview': {
      ...
      if (!filterByTk || !resourceName) ctx.throw(400, 'Missing filterByTk or resourceName');
      const tpl = await db.getRepository('print_templates').findOne({
        filter: { mainCollection: resourceName, enabled: true },
        sort: '-createdAt',
      });
...
function isSystemField(field: any): boolean {
  ...
  for (const p of SYSTEM_FIELD_PREFIXES) { if (name.startsWith(p)) return true; }
  for (const s of SYSTEM_FIELD_SUFFIXES) { if (name.endsWith(s)) return true; }
...
function buildFieldTree(db: any, fields: any[], currentDepth: number, maxDepth: number, visited: Set<string>, rootCollectionName: string): any[] {
      ...
      if (isRelation && f.options?.target && !visited.has(f.options.target)) {
        ...
        if (targetColl) {
          ...
          const subFields = toArray(targetColl.fields)
            .filter((sf: any) => !isSystemField(sf))
            .filter((sf: any) => {
              if (sf.type === 'belongsTo' && sf.options?.target === rootCollectionName) return false;
              if (sf.type === 'hasMany' && sf.options?.target === rootCollectionName) return false;
              if (sf.type === 'hasOne' && sf.options?.target === rootCollectionName) return false;
              if (sf.type === 'belongsToMany' && sf.options?.target === rootCollectionName) return false;
              return true;
            });
...
function getUserCollections(db: any): any[] {
  ...
  return arr.filter((col: any) => {
    ...
    if (name.startsWith('_')) return false;  // all system internal tables
    if (SYSTEM_COLLECTIONS.has(name)) return false;
    if (options.isThrough) return false;
    if (options.inherit === false && !options.title) return false;
    if (options.system === true || options.system === 'true') return false;
    if (options.hidden === true) return false;
    if (options.origin === 'pm' || options.origin === 'plugin') return false;
    const title = options.title || options.uiSchema?.title || name;
    if (typeof title === 'string' && title.startsWith('{{t(')) return false;
    if (title === name || title === options.name) return false;
    if (name.includes('_settings') || name.includes('_config')) return false;
    return true;
  });
}
...
function serveHtml(ctx: any, filePath: string) {
  ctx.withoutDataWrapping = true;
  ctx.type = 'text/html; charset=utf-8';
