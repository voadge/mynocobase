...
Page({
  ...
  onLoad(options) {
    ...
    const wxNickname = options.wxNickname ? decodeURIComponent(options.wxNickname) : '';
    this.setData({ openid, wxNickname }, () => {
      this.fetchUsers();
    });
  },
  ...
  doBind(userId) {
    ...
    wx.request({
      ...
      method: 'POST',
      data: { openid: this.data.openid, userId, wxNickname: this.data.wxNickname },
      success: (res) => {
        const data = res.data || {};
        if (data.code !== 0) {
