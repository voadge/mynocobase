...
export function registerDepartmentAcl(app: any, db: any) {
  app.acl.use(async (ctx: any, next: () => Promise<void>) => {
    try {
...
async function findMatchingRule(ctx: any, db: any) {
  ...
  const departmentIds = deptUsers.map((du: any) => du.departmentId);
  ...
  const actionMatch = (r: any) => {
    if (r.action === '*') return true;
    return (r.action || '').split(',').map((a: string) => a.trim()).includes(actionName);
  };
  const resMatch = rules.filter(actionMatch);
  if (resMatch.length === 0) return null;
  const exactMatch = resMatch.find(
    (r: any) => r.resourceName === resourceName && actionMatch(r)
  );
  const match = exactMatch || resMatch[0];
  if (match.mode === 'dept_and_role' && match.roleId) {
    const userRoleNames = ctx.state.currentUser?.roles?.map((r: any) => r.name) || [];
    if (!userRoleNames.includes(match.roleId)) return null;
  }
  return match;
}
