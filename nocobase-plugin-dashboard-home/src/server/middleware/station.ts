...
const _roadCache: { time: number; data: any[] } = { time: 0, data: [] };
const ROAD_CACHE_TTL = 300000;

async function loadRoads(db: any): Promise<any[]> {
  const now = Date.now();
  if (_roadCache.data.length && (now - _roadCache.time) < ROAD_CACHE_TTL) return _roadCache.data;
...
async function findNearestStation(
  ...
  lat: number,
  lng: number,
  opts: { roadId?: number; projectId?: number } = {}
): Promise<{
  road_id: number; road_code: string; road_name: string;
  station: string; station_m: number; distance: number;
  ...
  for (const road of roads) {
    ...
    if (effectiveDist > bufferMeters) continue;
    ...
    if (effectiveDist < bestDist) {
      ...
      bestResult = {
        ...
        road_code: road.code,
        road_name: road.name,
        station: stationToStr(stationM, prefix),
        station_m: stationM,
...
export function registerStationRoutes(app: any, authMiddleware: any): void {
  ...
  app.use(async (ctx: any, next: () => Promise<void>) => {
    ...
    try {
      ...
      if (!road) { ctx.body = { code: -1, msg: '路线不存在' }; return; }

      const points = parsePoints(road.points);
      if (points.length < 2) { ctx.body = { code: -1, msg: '路线点位不足' }; return; }

      // Find the segment containing this station_m
      const targetM = stationM - (road.station_offset_m || 0);
      let segIdx = 0;
      for (let i = 0; i < points.length - 1; i++) {
        if (points[i + 1].meters >= targetM) { segIdx = i; break; }
      ...
      ctx.body = { code: 0, data: { lat, lng, road_id: roadId, station: stationToStr(stationM, road.prefix || 'K') } };
    } catch (e: any) {
      ctx.body = { code: -1, msg: e.message };
    }
  }, { tag: 'dashboard-home', before: 'dataSource' });
  ...
  app.use(async (ctx: any, next: () => Promise<void>) => {
    ...
    try {
      ...
      const list = roads.map(r => ({
        ...
        point_count: (() => { try { return JSON.parse(r.points || '[]').length; } catch { return 0; } })(),
      }));
  ...
  app.use(async (ctx: any, next: () => Promise<void>) => {
    ...
    try {
      ...
      try {
        ...
        console.log('[station-import] auto-fence upsert failed:', (e as any).message);
      }
