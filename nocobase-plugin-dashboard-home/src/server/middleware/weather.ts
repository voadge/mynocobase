/**
 * QWeather (和风天气) proxy middleware
 */
import type { Context } from '@nocobase/server';
import { qwFetch, QW_WEATHER_HOST, QW_GEO_HOST } from '../utils/qw-jwt';

export function registerWeatherRoutes(app: any): void {
  ...
  app.use(async (ctx: Context, next: () => Promise<void>) => {
    ...
    try {
      ...
      if (lat && lng) {
        loc = lng + ',' + lat;
        try {
          const geo = await qwFetch('https://' + QW_GEO_HOST + '/geo/v2/city/lookup?location=' + encodeURIComponent(loc) + '&range=cn');
        ...
        try {
          const geo = await qwFetch('https://' + QW_GEO_HOST + '/geo/v2/city/lookup?location=' + encodeURIComponent(city) + '&range=cn');
          if (geo && geo.code === '200' && geo.location && geo.location[0]) {
      ...
      const w = await qwFetch('https://' + QW_WEATHER_HOST + '/v7/weather/now?location=' + encodeURIComponent(loc || city));
      if (w && w.code === '200') {
        const n = w.now || {};
        ctx.body = {
          code: 0, data: {
  ...
  app.use(async (ctx: Context, next: () => Promise<void>) => {
    ...
    try {
      const geo = await qwFetch('https://' + QW_GEO_HOST + '/geo/v2/city/lookup?location=' + encodeURIComponent(lng + ',' + lat) + '&range=cn');
      if (geo && geo.code === '200' && geo.location && geo.location[0]) {
