/**
 * Attendance submission, approval, and export middleware
 */
...
export function registerAttendanceRoutes(app: any, plugin: PluginInstance): void {
  const { db } = plugin;
  ...
  app.use(async (ctx: Context, next: () => Promise<void>) => {
    ...
    if (!await plugin.isAuthenticated(ctx)) { ctx.status = 401; ctx.body = 'Unauthorized'; return; }
    ctx.withoutDataWrapping = true;
