 ...
 */
export function convertToGCJ02(lat: number, lng: number, cs: 'gcj02' | 'wgs84'): [number, number] {
  if (cs === 'wgs84') return wgs84ToGcj02(lat, lng);
  return [lat, lng];
}
...
export function projectToSegment(
  ...
  lat2: number, lon2: number,
  cumMeters1: number
): { t: number; meters: number; distance: number } {
  const dAC = haversineDist(lat, lon, lat1, lon1);
  const dBC = haversineDist(lat, lon, lat2, lon2);
  const dAB = haversineDist(lat1, lon1, lat2, lon2);
  if (dAB < 1) return { t: 0, meters: cumMeters1, distance: dAC };
 ...
 */
export function nearestOnPolyline(lat: number, lon: number, polyline: PolylinePoint[]): PointOnSegment | null {
  if (!polyline || polyline.length < 2) return null;
  let best: PointOnSegment | null = null;
  let bestDist = Infinity;
  for (let i = 0; i < polyline.length - 1; i++) {
    const p = projectToSegment(
      lat, lon,
 ...
 */
export function stationToStr(stationM: number, prefix: string = 'K'): string {
  if (stationM < 0) {
 ...
 */
export function strToStation(str: string): { prefix: string; station_m: number } | null {
 ...
 */
export function recomputeMeters(points: Array<{ lng: number; lat: number; meters?: number }>): PolylinePoint[] {
