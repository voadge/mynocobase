...
const station_1 = require("./middleware/station");
...
module.exports = class DashboardHomePlugin extends server_1.Plugin {
    async load() {
        ...
        if (gfCol) {
            if (!gfCol.hasField('source'))
                gfCol.addField('source', { type: 'string', defaultValue: 'manual' });
            if (!gfCol.hasField('road_id'))
                gfCol.addField('road_id', { type: 'bigInt', nullable: true });
            gfCol.sync({ alter: true });
        }
        const arStationCol = db.getCollection('attendance_records');
        if (arStationCol) {
            if (!arStationCol.hasField('station'))
                arStationCol.addField('station', { type: 'string', nullable: true });
            if (!arStationCol.hasField('station_m'))
                arStationCol.addField('station_m', { type: 'double', nullable: true });
            if (!arStationCol.hasField('road_id'))
                arStationCol.addField('road_id', { type: 'bigInt', nullable: true });
            if (!arStationCol.hasField('structure_name'))
                arStationCol.addField('structure_name', { type: 'string', nullable: true });
            arStationCol.sync({ alter: true });
        }
        const lhCol = db.getCollection('location_history');
        if (lhCol) {
            if (!lhCol.hasField('station'))
                lhCol.addField('station', { type: 'string', nullable: true });
            if (!lhCol.hasField('station_m'))
                lhCol.addField('station_m', { type: 'double', nullable: true });
            if (!lhCol.hasField('road_id'))
                lhCol.addField('road_id', { type: 'bigInt', nullable: true });
            if (!lhCol.hasField('cs'))
                lhCol.addField('cs', { type: 'string', nullable: true, defaultValue: 'gcj02' });
        ...
        (0, station_1.registerStationRoutes)(app, { isAuthenticated: auth_1.isAuthenticated.bind(this) });
...
b.addEventListener('click',async function(){var code=c.value.trim(),dt=d.value;if(!code||!dt){alert('璇峰～鍐欓」鐩紪鍙峰拰鏃ユ湡');return}var ymd=parseInt(dt.replace(/-/g,''));b.disabled=true;b.textContent='姹囨€讳�?..';try{var r=await fetch('/api/__pd__/aggregate-log',{method:'POST',headers:{'Content-Type':'application/json'},credentials:'same-origin',body:JSON.stringify({projectNameNo:code,date:ymd})});var j=await r.json();if(j.code===0&&j.data?.updated)alert('姹囨€诲畬鎴愶紝鏂板�?'+j.data.newEntryCount+' �?);else if(j.code===0)alert(j.data?.message||'娌℃湁鏂板唴瀹归渶瑕佹眹�?);else alert('姹囨€诲け璐ワ細'+(j.msg||'鏈煡閿欒'));rf()}catch(e){alert('姹囨€诲け璐? '+e.message)}finally{b.disabled=false;b.textContent='\u26A1 姹囨€绘棩蹇?}});
        ...
        if (roadCol) {
            const recomputeAndUpsert = async (record) => {
                try {
                    ...
                    if (record.get('is_fence_active')) {
                        ...
                        const lngs = recomputed.map((p) => p.lng);
                        const lats = recomputed.map((p) => p.lat);
                        ...
                        const fenceVals = {
                            ...
                            polyline_coords: JSON.stringify(recomputed.map((p) => [p.lng, p.lat])),
                    ...
                    console.log('[road-hook] error:', e.message);
                }
            };
        ...
        const stationHooks = async (record) => {
            try {
                if (record.get('station'))
                    return; // C7: anti-recursion guard
                ...
                for (const road of roads) {
                    ...
                    if (!nearest || nearest.distance > (road.buffer_meters || 200))
                        continue;
                ...
                console.log('[station-hook] error:', e.message);
            }
        };
        const lhStationCol = db.getCollection('location_history');
        if (lhStationCol)
            lhStationCol.model.addHook('afterCreate', stationHooks);
        const arStationHook = db.getCollection('attendance_records');
        if (arStationHook)
            arStationHook.model.addHook('afterCreate', stationHooks);
