...
exports.registerDeptAdminApi = registerDeptAdminApi;
const dept_helpers_1 = require("./dept-helpers");
function registerDeptAdminApi(app, plugin) {
    const { db } = plugin;
    ...
    app.use(async (ctx, next) => {
        if (ctx.method !== 'GET' || ctx.state.reqPath !== '/__da__/acl-rules')
            return await next();
        ...
        try {
            ...
            if (ctx.query.departmentId)
                filter.departmentId = parseInt(ctx.query.departmentId);
    ...
    app.use(async (ctx, next) => {
        ...
        try {
            ...
            const collectionFields = (col) => {
                if (!col)
                    return [];
                const names = new Set(Array.from(col.fields.keys()));
            ...
            try {
                ...
                db.collections.forEach((col) => {
                    if (col.isThrough && col.isThrough())
                        return;
                    items.push({
                        name: col.name,
    ...
    app.use(async (ctx, next) => {
        ...
        try {
            ...
            const levelKey = ctx.query.levelKey || '';
            if (!deptId || !levelKey) {
                ctx.status = 400;
                ctx.body = { error: 'departmentId and levelKey are required' };
                return;
            }
            const approvers = await (0, dept_helpers_1.resolveApproversByRoute)(deptId, levelKey, db);
