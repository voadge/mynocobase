...
exports.registerDeptAdminPages = registerDeptAdminPages;
function adminPageHtml(title, tableConfig, extraJs) {
    return `<!DOCTYPE html>
...
function registerDeptAdminPages(app) {
    // ACL rules management page
    app.use(async (ctx, next) => {
        if (ctx.method !== 'GET' || ctx.state.reqPath !== '/__da__/acl-ui')
            return await next();
        ctx.withoutDataWrapping = true;
        ctx.type = 'text/html; charset=utf-8';
        ctx.body = adminPageHtml('部门权限管理', `<button onclick="editItem(null)">+ 新建规则</button><label style="margin-left:auto">部门筛选：<select id="deptFilter" onchange="loadData()"><option value="">全部部门</option></select></label>`, `
...
loadRefs();`);
    ...
    app.use(async (ctx, next) => {
        ...
        ctx.withoutDataWrapping = true;
        ctx.type = 'text/html; charset=utf-8';
        ctx.body = adminPageHtml('审批路由管理', `<button onclick="editItem(null)">+ 新建路由</button>`, `
async function loadData() {
